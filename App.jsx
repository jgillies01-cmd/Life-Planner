...
.nb {
  ...
  color:#9CA3AF; font-family:'Instrument Sans',sans-serif; font-size:12.5px; font-weight:500;
...
// ─── STORAGE ──────────────────────────────────────────────────────────────────
// localStorage wrapper that matches the window.storage interface used in the
// Claude artifact. Swap window.storage for this and the rest of the code is
// identical between the artifact and the hosted app.
const store = {
  async get(key) {
    try {
...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const EV_COLORS = { work:'#2563EB', personal:'#7C3AED', fitness:'#059669', health:'#DC2626', social:'#D97706' };
...
export default function LifeOS() {
  ...
  // AI — calls the /api/ai serverless proxy, never Anthropic directly
  ...
  // CRUD
  const addEvent=async()=>{ if(!evForm.title.trim())return; const u=[...events,{id:Date.now(),...evForm,date:new Date().toISOString()}]; await store.set('los_e',JSON.stringify(u)); setEvents(u); setEvForm({title:'',time:'',type:'work'}); setShowAddEv(false); };
  ...
  return (
    <div className="app">
      ...
      <div className="pg">
        ...
        {view==='today'&&(
          <div className="fi">
            ...
            <div className="sg">
              <div className="sc" style={{'--accent':'#2563EB'}}><div className="sc-lbl">Events</div><div className="sc-val" style={{color:'#2563EB'}}>{te.length}</div><div className="sc-sub">today</div></div>
              <div className="sc" style={{'--accent':calOver?'#DC2626':'#16A34A'}}><div className="sc-lbl">Calories</div><div className="sc-val" style={{color:calOver?'#DC2626':'#16A34A'}}>{todayCals}</div><div className="sc-sub">of {calorieGoal}</div></div>
              <div className="sc" style={{'--accent':'#7C3AED'}}><div className="sc-lbl">Goals</div><div className="sc-val" style={{color:'#7C3AED'}}>{activeGoals.length}</div><div className="sc-sub">{doneGoals.length} done</div></div>
            </div>
            <div className="card">
              <div className="card-hdr">
                <div className="ctitle" style={{color:'#15803D'}}><span className="cdot" style={{background:'#16A34A'}}/>AI Daily Brief</div>
                <button className="btn btn-ghost btn-sm" onClick={genBrief} disabled={briefLoad}>{briefLoad?<><span className="d1">●</span><span className="d2">●</span><span className="d3">●</span></>:<><RefreshCw size={10}/> Generate</>}</button>
              </div>
              {briefLoad?<p style={{color:'#9CA3AF',fontSize:'13px'}}>Claude is crafting your brief<span className="d1">.</span><span className="d2">.</span><span className="d3">.</span></p>
                :brief?<div className="brief-wrap"><p className="brief-text">{brief}</p></div>
                :<div className="empty"><div style={{fontSize:'28px',marginBottom:10}}>🌿</div><p style={{marginBottom:10,fontFamily:'Spectral,Georgia,serif',fontStyle:'italic',fontSize:'13.5px',color:'#6B7280',lineHeight:1.7}}>Generate your personal daily brief<br/>to start the day with clarity.</p><button className="btn btn-gold btn-sm" onClick={genBrief}>Generate My Brief</button></div>}
            </div>
            <div className="card">
              <div className="card-hdr"><div className="ctitle" style={{color:'#1D4ED8'}}><span className="cdot" style={{background:'#2563EB'}}/>Today's Schedule</div><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddEv(true)}><Plus size={10}/> Add</button></div>
              {te.length===0?<div className="empty"><Calendar size={22}/><p style={{marginBottom:9}}>No events yet.</p><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddEv(true)}>+ Add Event</button></div>
                :te.map(ev=>(
        ...
        {view==='nutrition'&&(
          <div className="fi">
            ...
            <div className="card">
              ...
              <div style={{display:'flex',alignItems:'baseline',gap:7,marginBottom:10}}><span className="syn" style={{fontSize:42,fontWeight:800,lineHeight:1,color:calOver?'#DC2626':'#16A34A'}}>{todayCals}</span><span style={{color:'#9CA3AF',fontSize:13}}>/ {calorieGoal} kcal</span><span style={{marginLeft:'auto',fontSize:11,fontFamily:'JetBrains Mono,monospace',color:calOver?'#DC2626':'#16A34A'}}>{calPct}%</span></div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
                {[['Protein','#2563EB',todayProtein,150],['Carbs','#D97706',todayCarbs,250],['Fat','#7C3AED',todayFat,70]].map(([lbl,col,val,goal])=>(
                  <div key={lbl}><div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}><span className="mono" style={{fontSize:9,color:'#9CA3AF',textTransform:'uppercase',letterSpacing:'.13em'}}>{lbl}</span><span className="mono" style={{fontSize:9,color:col}}>{val}g</span></div><div className="prog-bar" style={{height:4,background:'#EEF3EC'}}><div className="prog-fill" style={{width:`${Math.min(100,Math.round(val/goal*100))}%`,background:col}}/></div></div>
                ))}
              </div>
            </div>
            <div className="card">
              ...
              {tf.length===0?<div className="empty"><Utensils size={22}/><p style={{marginBottom:9}}>Nothing logged yet.</p><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddFood(true)}>+ Log First Meal</button></div>
        ...
        {view==='goals'&&(
          <div className="fi">
            ...
            <div className="card">
              ...
              {activeGoals.length===0?<div className="empty"><Target size={22}/><p style={{marginBottom:9}}>No active goals yet.</p><button className="btn btn-gold btn-sm" onClick={()=>setShowAddGl(true)}>+ Add First Goal</button></div>
        ...
        {view==='settings'&&(
          <div className="fi">
            ...
            {[{title:'📅 Google Calendar',color:'#2563EB',badge:'bg-blu',label:'Phase 2',desc:'Pulls today\'s events and meetings automatically.',steps:[{t:'Create a Google Cloud Project',d:'console.cloud.google.com → New Project → "LifeOS".'},{t:'Enable Calendar API',d:'APIs & Services → Library → "Google Calendar API" → Enable.'},{t:'Create OAuth credentials',d:'Credentials → OAuth 2.0 Client ID → Web Application.'},{t:'Add your Vercel URL',d:'Authorised redirect URIs → https://your-app.vercel.app/auth/google/callback'},{t:'Add keys to Vercel',d:'Environment Variables → GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET.'},{t:'Ask Claude to add it',d:'Tell Claude: "Add Google Calendar OAuth to my hosted LifeOS."'}]},
            ...
            <div className="card"><div className="card-hdr"><div className="ctitle"><span className="cdot"/>Reset</div></div><p style={{fontSize:'11.5px',color:'#9CA3AF',marginBottom:'10px'}}>Clear all saved data. Cannot be undone.</p><button className="btn btn-danger btn-sm" onClick={()=>{if(confirm('Reset all data?')){['los_p','los_g','los_e','los_b','los_c','los_f','los_cal'].forEach(k=>localStorage.removeItem(k));window.location.reload();}}}><Trash2 size={11}/> Reset LifeOS</button></div>
          </div>
        )}
      </div>
      ...
      {showAddEv&&(<div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget)setShowAddEv(false);}}><div className="modal"><h3>Add Event</h3><input className="inp" placeholder="Event title…" value={evForm.title} onChange={e=>setEvForm(f=>({...f,title:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&addEvent()} autoFocus/><input className="inp" type="time" value={evForm.time} onChange={e=>setEvForm(f=>({...f,time:e.target.value}))}/><select className="sel" value={evForm.type} onChange={e=>setEvForm(f=>({...f,type:e.target.value}))}><option value="work">💼 Work</option><option value="personal">🏠 Personal</option><option value="fitness">🏃 Fitness</option><option value="health">❤️ Health</option><option value="social">👥 Social</option></select><div className="row-btns"><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddEv(false)}>Cancel</button><button className="btn btn-gold btn-sm" onClick={addEvent}>Add</button></div></div></div>)}
