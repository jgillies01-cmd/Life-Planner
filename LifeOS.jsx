...
import { Calendar, Activity, Utensils, Briefcase, Settings, MessageCircle, Plus, X, RefreshCw, Check, Target, Trash2, Zap, Search } from "lucide-react";
...
// ─── DESIGN DIRECTION ─────────────────────────────────────────────────────────
// "Botanical precision" — a fresh, bright life OS.
// Clean white cards on a warm grey-green canvas. Deep forest green anchors
// the brand; a vivid mid-green drives actions and progress. Warm greys
// carry all text hierarchy. The Spectral serif brief remains the editorial
// centrepiece — now dark ink on white, like a personal morning paper.
const CSS = `
...
.app {
  ...
  background-image:
    radial-gradient(ellipse 65% 40% at 0% 0%,   rgba(22,163,74,.1)  0%, transparent 60%),
    radial-gradient(ellipse 50% 35% at 100% 100%, rgba(13,102,64,.07) 0%, transparent 55%),
    radial-gradient(circle, rgba(0,0,0,.038) 1px, transparent 1px);
...
.logo {
  ...
  letter-spacing:.15em; color:#0A5C38; user-select:none;
  display:flex; align-items:center; gap:8px;
}
.logo::before {
  content:''; display:block; width:3px; height:16px;
  background:#0A5C38; border-radius:2px; flex-shrink:0;
}
...
.nb {
  padding:7px 13px 11px; background:none; border:none;
  border-bottom:2px solid transparent;
  color:#9CA3AF; font-family:'Instrument Sans',sans-serif;
...
.card {
  ...
  padding: 15px;
  margin-bottom: 11px;
  box-shadow: 0 1px 3px rgba(0,0,0,.04), 0 1px 0 rgba(255,255,255,.9);
  transition: border-color .22s, box-shadow .22s, transform .22s;
}
.card:hover {
  border-color: #BED4BA;
  box-shadow: 0 4px 16px rgba(22,163,74,.08), 0 1px 3px rgba(0,0,0,.04);
...
.ibtn {
  ...
  background:#F6FAF5; border:1px solid #D1D5DB; color:#4B5563;
...
.sc {
  background:#FFFFFF; border:1px solid #DDE8DA; border-radius:10px;
...
.sc::after {
  ...
  background:var(--accent,transparent); border-radius:10px 10px 0 0; opacity:.75;
}
...
/* ── Brief — editorial serif ─────────────────────────────────────────────── */
.brief-wrap { position:relative; padding-left:18px; }
.brief-wrap::before {
  content:''; position:absolute; left:0; top:4px; bottom:4px;
  width:2px; background:rgba(10,92,56,.4); border-radius:2px;
}
.brief-text {
  font-family:'Spectral',Georgia,serif; font-size:14.5px;
  line-height:1.85; color:#374151; white-space:pre-wrap; letter-spacing:.01em;
}
...
/* ── Events ──────────────────────────────────────────────────────────────── */
.ev { display:flex; align-items:center; gap:8px; padding:8px 0; border-bottom:1px solid #EEF3EC; }
.ev:last-child { border-bottom:none; }
.ev-time { font-family:'JetBrains Mono',monospace; font-size:10px; color:#9CA3AF; width:36px; flex-shrink:0; }
...
.chat-pnl {
  width:100%; height:78vh; background:#FFFFFF;
  border:1px solid #DDE8DA; border-bottom:none;
...
.fab {
  ...
  cursor:pointer; display:flex; align-items:center; justify-content:center;
  box-shadow:0 4px 20px rgba(22,163,74,.38); z-index:40;
  transition:all .22s;
}
...
.warn-box { background:rgba(180,83,9,.05);  border:1px solid rgba(180,83,9,.13);  border-radius:8px; padding:10px 13px; font-size:11.5px; color:#92400E; line-height:1.65; margin-bottom:11px; }
...
// Darker event-type colours for readability on white cards
const EV_COLORS = {
  work:     '#2563EB',
  personal: '#7C3AED',
  fitness:  '#059669',
...
export default function LifeOS() {
  ...
  const [evForm, setEvForm]           = useState({ title:'', time:'', type:'work' });
  ...
  useEffect(() => {
    ...
    inited.current = true;
    (async () => {
      try { const r=await window.storage.get('los_p'); if(r){const p=JSON.parse(r.value);setName(p.n);setReady(true);} } catch {}
      try { const r=await window.storage.get('los_g'); if(r) setGoals(JSON.parse(r.value)); } catch {}
      try { const r=await window.storage.get('los_e'); if(r) setEvents(JSON.parse(r.value)); } catch {}
      try { const r=await window.storage.get('los_b'); if(r){const b=JSON.parse(r.value);if(b.d===new Date().toDateString())setBrief(b.t);} } catch {}
      try { const r=await window.storage.get('los_c'); if(r) setChat(JSON.parse(r.value)); } catch {}
      try { const r=await window.storage.get('los_f'); if(r) setFoodLog(JSON.parse(r.value)); } catch {}
      try { const r=await window.storage.get('los_cal'); if(r) setCalorieGoal(JSON.parse(r.value)); } catch {}
    })();
  }, []);
  ...
  const doSetup = async () => {
    ...
    await window.storage.set('los_p',JSON.stringify({n}));
    setName(n); setReady(true);
  };

  const todayEvs = () => {
    const td=new Date().toDateString();
    return events.filter(e=>!e.date||new Date(e.date).toDateString()===td)
  ...
  const genBrief = async () => {
    ...
    try {
      ...
      setBrief(t);
      await window.storage.set('los_b',JSON.stringify({t,d:new Date().toDateString()}));
  ...
  const sendChat = async (override) => {
    ...
    try {
      ...
      setChat(final);
      await window.storage.set('los_c',JSON.stringify(final.slice(-30)));
    } catch { setChat([...msgs,{role:'assistant',content:'Connection error — try again.'}]); }
    setChatLoad(false);
  };

  // ── CRUD ───────────────────────────────────────────────────────────────
  const addEvent=async()=>{ if(!evForm.title.trim())return; const u=[...events,{id:Date.now(),...evForm,date:new Date().toISOString()}]; await window.storage.set('los_e',JSON.stringify(u)); setEvents(u); setEvForm({title:'',time:'',type:'work'}); setShowAddEv(false); };
  const delEvent=async(id)=>{ const u=events.filter(e=>e.id!==id); await window.storage.set('los_e',JSON.stringify(u)); setEvents(u); };
  const addGoal=async()=>{ if(!glIn.trim())return; const u=[...goals,{id:Date.now(),text:glIn.trim(),done:false}]; await window.storage.set('los_g',JSON.stringify(u)); setGoals(u); setGlIn(''); setShowAddGl(false); };
  const toggleGoal=async(id)=>{ const u=goals.map(g=>g.id===id?{...g,done:!g.done}:g); await window.storage.set('los_g',JSON.stringify(u)); setGoals(u); };
  const delGoal=async(id)=>{ const u=goals.filter(g=>g.id!==id); await window.storage.set('los_g',JSON.stringify(u)); setGoals(u); };
  ...
  const addSearchedFood=async()=>{ if(!selectedFood||!portionG)return; const g=parseFloat(portionG); if(isNaN(g)||g<=0)return; const food={id:Date.now(),name:`${selectedFood.name} (${portionG}g)`,calories:Math.round(selectedFood.cal*g/100),protein:Math.round(selectedFood.prot*g/100),carbs:Math.round(selectedFood.carb*g/100),fat:Math.round(selectedFood.fat*g/100),meal:searchMeal,date:new Date().toISOString()}; const u=[...foodLog,food]; await window.storage.set('los_f',JSON.stringify(u)); setFoodLog(u); setShowAddFood(false); setSelectedFood(null); setPortionG(''); setFoodQuery(''); setSearchResults([]); setSearchStatus('idle'); };
  const addManualFood=async()=>{ if(!manualFood.name.trim()||!manualFood.calories)return; const food={id:Date.now(),name:manualFood.name.trim(),calories:parseInt(manualFood.calories)||0,protein:parseFloat(manualFood.protein)||0,carbs:parseFloat(manualFood.carbs)||0,fat:parseFloat(manualFood.fat)||0,meal:manualFood.meal,date:new Date().toISOString()}; const u=[...foodLog,food]; await window.storage.set('los_f',JSON.stringify(u)); setFoodLog(u); setManualFood({name:'',calories:'',protein:'',carbs:'',fat:'',meal:'lunch'}); setShowAddFood(false); };
  const delFood=async(id)=>{ const u=foodLog.filter(f=>f.id!==id); await window.storage.set('los_f',JSON.stringify(u)); setFoodLog(u); };
  const updateGoalCalories=async()=>{ const val=parseInt(prompt('Daily calorie goal (kcal):',calorieGoal)); if(!isNaN(val)&&val>0){await window.storage.set('los_cal',JSON.stringify(val));setCalorieGoal(val);} };
  ...
  return (
    <div className="app">
      ...
      <div className="pg">
          ...
          <div className="fi">
            ...
            <div className="sg">
              <div className="sc" style={{'--accent':'#2563EB'}}>
                ...
                <div className="sc-val" style={{color:'#2563EB'}}>{te.length}</div>
            ...
            <div className="card">
              <div className="card-hdr">
                <div className="ctitle" style={{color:'#1D4ED8'}}><span className="cdot" style={{background:'#2563EB'}}/>Today's Schedule</div>
        ...
        {view==='nutrition'&&(
          <div className="fi">
            ...
            <div className="card">
              <div className="card-hdr">
                <div className="ctitle" style={{color:'#15803D'}}><span className="cdot" style={{background:'#16A34A'}}/>Calories Today</div>
                <button className="btn btn-ghost btn-sm" onClick={updateGoalCalories}>Goal: {calorieGoal} kcal ✎</button>
              </div>
              ...
              <div className="prog-bar" style={{marginBottom:16}}>
                <div className="prog-fill" style={{width:`${calPct}%`,background:calOver?'#DC2626':'#16A34A'}}/>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
                {[['Protein','#2563EB',todayProtein,150],['Carbs','#D97706',todayCarbs,250],['Fat','#7C3AED',todayFat,70]].map(([lbl,col,val,goal])=>(
                  <div key={lbl}>
                    ...
                    <div className="prog-bar" style={{height:4,background:'#EEF3EC'}}>
                      <div className="prog-fill" style={{width:`${Math.min(100,Math.round(val/goal*100))}%`,background:col}}/>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="card">
              <div className="card-hdr">
                <div className="ctitle" style={{color:'#15803D'}}><span className="cdot" style={{background:'#16A34A'}}/>Today's Log</div>
                <button className="btn btn-gold btn-sm" onClick={()=>setShowAddFood(true)}><Plus size={10}/> Log Food</button>
              </div>
              {tf.length===0
                ...
                :MEALS.map(meal=>{
                    const items=tf.filter(f=>f.meal===meal); if(!items.length) return null;
                    const mc=items.reduce((s,f)=>s+(f.calories||0),0);
                    return(
                      <div key={meal}>
                        <div className="meal-lbl">{MEAL_ICONS[meal]} {meal} · <span style={{color:'#6B7280'}}>{mc} kcal</span></div>
                        {items.map(food=>(
                          <div key={food.id} className="food-row">
                            <span style={{flex:1,fontSize:13,color:'#1F2937'}}>{food.name}</span>
                            <span className="mono" style={{fontSize:10,color:'#16A34A',flexShrink:0}}>{food.calories} kcal</span>
                            {food.protein>0&&<span className="mono" style={{fontSize:9,color:'#2563EB',flexShrink:0}}>{food.protein}g P</span>}
                            <button className="food-del" onClick={()=>delFood(food.id)}><X size={11}/></button>
                          </div>
                        ))}
                      </div>
                    );
        ...
        {view==='goals'&&(
          <div className="fi">
            ...
            <div className="card">
              <div className="card-hdr">
                <div className="ctitle" style={{color:'#15803D'}}><span className="cdot" style={{background:'#16A34A'}}/>Active ({activeGoals.length})</div>
                <button className="btn btn-ghost btn-sm" onClick={()=>setShowAddGl(true)}><Plus size={10}/> Add</button>
              </div>
              {activeGoals.length===0
                ...
                :activeGoals.map(g=>(
                  <div key={g.id} className="gl">
                    <div className="gl-ck" onClick={()=>toggleGoal(g.id)}/>
                    <span className="gl-txt">{g.text}</span>
                    <button className="gl-del" onClick={()=>delGoal(g.id)}><Trash2 size={12}/></button>
                  </div>
                ))}
            </div>
            {doneGoals.length>0&&(
              <div className="card">
                <div className="card-hdr"><div className="ctitle"><span className="cdot"/>Completed ({doneGoals.length})</div></div>
                {doneGoals.map(g=>(
                  <div key={g.id} className="gl">
                    <div className="gl-ck done" onClick={()=>toggleGoal(g.id)}><Check size={9} color="#FFFFFF"/></div>
                    <span className="gl-txt done">{g.text}</span>
                    <button className="gl-del" onClick={()=>delGoal(g.id)}><Trash2 size={12}/></button>
                  </div>
                ))}
              </div>
            )}
            <div className="card" style={{borderColor:'rgba(124,58,237,.2)'}}>
              <div className="card-hdr"><div className="ctitle" style={{color:'#6D28D9'}}><span className="cdot" style={{background:'#7C3AED'}}/>Get AI Goal Ideas</div></div>
              <p style={{fontSize:'12px',color:'#6B7280',marginBottom:'10px',lineHeight:1.65}}>Ask Claude to help you define meaningful goals around fitness, nutrition, career or wellbeing.</p>
              <button className="btn btn-ghost btn-sm" onClick={()=>{setView('planner');sendChat('Help me set meaningful goals for this month');}}>Ask Claude →</button>
            </div>
          </div>
        )}
            ...
            <div className="plan-msgs">
              {chat.length===0&&(
                <div style={{textAlign:'center',padding:'28px 12px',color:'#9CA3AF'}}>
                  <Zap size={30} style={{marginBottom:10,opacity:.3}}/>
                  <p style={{fontSize:'13.5px',marginBottom:'18px',lineHeight:1.75,fontFamily:'Spectral,Georgia,serif',fontStyle:'italic',color:'#6B7280'}}>Ask me to plan your day, build routines,<br/>set weekly goals, or hold you accountable.</p>
                  <div style={{display:'flex',flexDirection:'column',gap:'6px',maxWidth:'310px',margin:'0 auto'}}>
                    {['Plan my ideal morning routine','Help me plan this week productively','What habits should I build?','Create a workout and nutrition schedule'].map(s=>(
                      <button key={s} className="btn btn-ghost" style={{justifyContent:'flex-start',fontSize:'12px',textAlign:'left'}} onClick={()=>sendChat(s)}>{s}</button>
                    ))}
                  </div>
                </div>
              )}
        ...
        {view==='settings'&&(
          <div className="fi">
            ...
            <div className="card">
              ...
              <button className="btn btn-ghost btn-sm" onClick={async()=>{const n=prompt('Your name:',name);if(n?.trim()){await window.storage.set('los_p',JSON.stringify({n:n.trim()}));setName(n.trim());}}}>Edit Name</button>
            </div>
            <div className="card" style={{borderColor:'rgba(22,163,74,.28)'}}>
              <div className="card-hdr"><div className="ctitle" style={{color:'#15803D'}}><span className="cdot" style={{background:'#16A34A'}}/>🌐 Step 1 — Host Online First</div><span className="badge bg-grn">Free</span></div>
              <div className="info-box">Once hosted, you get a permanent URL to open on your phone and laptop, and can connect all integrations.</div>
              {[{t:'Get the code from Claude',d:'In a new chat ask: "Give me the full standalone React code for my Life OS dashboard, ready to deploy on Vercel."'},
                {t:'Create a GitHub account',d:'github.com → Sign Up (free).'},
                {t:'Create a new repository',d:'+ → New Repository → name it "lifeos" → Public → Create.'},
                {t:'Upload your files',d:'Click "uploading an existing file" → drag in the files → Commit changes.'},
                {t:'Sign up for Vercel',d:'vercel.com → Continue with GitHub.'},
                {t:'Deploy',d:'New Project → Import "lifeos" → Deploy. Live in ~1 minute.'},
                {t:'Add to Android home screen',d:'Open the URL in Chrome → ⋮ → Add to Home Screen.'},
              ].map((s,i)=>(<div key={i} className="step"><div className="snum">{i+1}</div><div><div className="stitle">{s.t}</div><div className="sdesc">{s.d}</div></div></div>))}
            </div>
            {[{title:'📅 Google Calendar',color:'#2563EB',badge:'bg-blu',label:'Phase 2',desc:'Pulls today\'s events and meetings into your schedule automatically.',steps:[{t:'Create a Google Cloud Project',d:'console.cloud.google.com → New Project → "LifeOS".'},{t:'Enable Calendar API',d:'APIs & Services → Library → "Google Calendar API" → Enable.'},{t:'Create OAuth credentials',d:'Credentials → OAuth 2.0 Client ID → Web Application.'},{t:'Add your Vercel URL',d:'Authorised redirect URIs → https://your-app.vercel.app/auth/google/callback'},{t:'Add keys to Vercel',d:'Environment Variables → GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET.'},{t:'Ask Claude to add it',d:'Tell Claude: "Add Google Calendar OAuth to my hosted LifeOS."'}]},
              {title:'🏃 Strava',color:'#059669',badge:'bg-grn',label:'Phase 3',desc:'Pulls in your runs, rides and other activities.',steps:[{t:'Create a Strava API app',d:'strava.com/settings/api → Create App → use your Vercel URL.'},{t:'Note your credentials',d:'Copy the Client ID and Client Secret.'},{t:'Add to Vercel',d:'Environment Variables → STRAVA_CLIENT_ID and STRAVA_CLIENT_SECRET.'},{t:'Ask Claude to add it',d:'Tell Claude: "Add Strava OAuth to my LifeOS."'}]},
              {title:'💼 RotaCloud',color:'#7C3AED',badge:'bg-pur',label:'Phase 4',desc:'Syncs your work shifts into your daily schedule.',steps:[{t:'Get your RotaCloud API key',d:'Settings → Integrations → API Key → Generate.'},{t:'Add to Vercel',d:'Environment Variables → ROTACLOUD_API_KEY.'},{t:'Ask Claude to add it',d:'Tell Claude: "Add RotaCloud to my LifeOS to pull in my work shifts."'}]},
              {title:'🎙 Telegram Voice Notes',color:'#D97706',badge:'bg-warn',label:'Phase 5',desc:'Send voice notes to a Telegram bot — Claude transcribes them and adds events, goals and notes to your dashboard automatically.',steps:[{t:'Create a Telegram bot',d:'Open Telegram → message @BotFather → /newbot → follow the steps. Copy your bot token.'},{t:'Sign up for Make.com',d:'make.com → Free account. Your no-code automation engine.'},{t:'Build the scenario',d:'New Scenario → Telegram trigger → OpenAI Whisper (transcribe audio) → Claude (extract structured data).'},{t:'Set up Supabase',d:'supabase.com → New Project → tables for events, goals and notes. Make stores Claude\'s output here.'},{t:'Ask Claude to connect Life OS',d:'Tell Claude: "Update my LifeOS to read from Supabase in addition to local storage."'}]}
            ].map(int=>(
            ...
            <div className="card">
              ...
              <button className="btn btn-danger btn-sm" onClick={async()=>{if(confirm('Reset all data?')){for(const k of['los_p','los_g','los_e','los_b','los_c','los_f','los_cal'])try{await window.storage.delete(k)}catch{}window.location.reload();}}}><Trash2 size={11}/> Reset LifeOS</button>
            </div>
          </div>
        )}
      </div>
      ...
      {/* CHAT OVERLAY */}
      {showChat&&(
        <div className="chat-bg" onClick={e=>{if(e.target===e.currentTarget)setShowChat(false);}}>
          <div className="chat-pnl">
            <div className="chat-hdr">
              <div>
                <div className="syn" style={{fontWeight:700,fontSize:'13.5px',color:'#111827'}}>Quick Planner</div>
                <div className="mono" style={{fontSize:'9.5px',color:'#9CA3AF'}}>Powered by Claude</div>
              </div>
              <button className="ibtn" onClick={()=>setShowChat(false)}><X size={13}/></button>
            </div>
            <div className="chat-msgs"><ChatMessages endRef={chatBot}/></div>
            <div className="chat-in">
              <textarea className="cin" rows={2} placeholder="Plan my day, set goals, review my week…" value={chatIn} onChange={e=>setChatIn(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendChat();}}} style={{resize:'none'}}/>
              <button className="btn btn-gold" onClick={()=>sendChat()} disabled={chatLoad} style={{alignSelf:'flex-end'}}>→</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ADD EVENT */}
      {showAddEv&&(
        <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget)setShowAddEv(false);}}>
          <div className="modal"><h3>Add Event</h3>
      ...
      {/* ADD GOAL */}
      {showAddGl&&(
        <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget)setShowAddGl(false);}}>
          <div className="modal"><h3>Add Goal</h3>
            <input className="inp" placeholder="e.g. Run 3× per week…" value={glIn} onChange={e=>setGlIn(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addGoal()} autoFocus/>
            <div className="row-btns"><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddGl(false)}>Cancel</button><button className="btn btn-gold btn-sm" onClick={addGoal}>Add Goal</button></div>
          </div>
        </div>
      )}
      ...
      {/* ADD FOOD */}
      {showAddFood&&(
        <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget)closeAddFood();}}>
          <div className="modal" style={{maxWidth:400}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:13}}>
              <h3 style={{margin:0}}>Log Food</h3>
              <div style={{display:'flex',gap:5}}>
                <button className={`btn btn-sm ${addMode==='search'?'btn-gold':'btn-ghost'}`} onClick={()=>{setAddMode('search');setSelectedFood(null);setPortionG('');}}><Search size={10}/> Search</button>
                <button className={`btn btn-sm ${addMode==='manual'?'btn-gold':'btn-ghost'}`} onClick={()=>setAddMode('manual')}>✎ Manual</button>
              </div>
            </div>
            {addMode==='search'&&!selectedFood&&(
              <>
                <div style={{display:'flex',gap:7,marginBottom:9}}>
                  <input className="inp" placeholder="Search foods, brands…" value={foodQuery} onChange={e=>setFoodQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&searchFoods()} autoFocus style={{margin:0,flex:1}}/>
                  <button className="btn btn-ghost btn-sm" onClick={searchFoods} disabled={searchStatus==='loading'} style={{flexShrink:0}}>
                    {searchStatus==='loading'?<><span className="d1">●</span><span className="d2">●</span><span className="d3">●</span></>:<Search size={12}/>}
                  </button>
                </div>
                <p style={{fontSize:'10.5px',color:'#9CA3AF',marginBottom:9,lineHeight:1.5}}>Powered by Open Food Facts — great for packaged foods. Use Manual for fresh ingredients.</p>
                {searchStatus==='done'&&searchResults.map((r,i)=>(
                  <div key={i} className="sr" onClick={()=>setSelectedFood(r)}>
                    <span style={{fontSize:12,flex:1,lineHeight:1.4,color:'#1F2937'}}>{r.name}</span>
                    <span className="mono" style={{fontSize:9.5,color:'#16A34A',flexShrink:0}}>{r.cal} kcal/100g</span>
                  </div>
                ))}
                {searchStatus==='empty'&&<p style={{fontSize:'12px',color:'#9CA3AF',padding:'8px 0'}}>No results. Try different terms or use Manual entry.</p>}
                {searchStatus==='error'&&<p style={{fontSize:'12px',color:'#DC2626',padding:'8px 0'}}>Search failed — check connection or use Manual.</p>}
                <div className="row-btns" style={{marginTop:6}}><button className="btn btn-ghost btn-sm" onClick={closeAddFood}>Cancel</button></div>
              </>
            )}
            {addMode==='search'&&selectedFood&&(
              <>
                <div style={{background:'#F7FAF6',border:'1px solid #DDE8DA',borderRadius:8,padding:'10px 12px',marginBottom:11}}>
                  <div style={{fontSize:12.5,fontWeight:600,marginBottom:4,color:'#111827'}}>{selectedFood.name}</div>
                  <div className="mono" style={{fontSize:9.5,color:'#9CA3AF'}}>Per 100g: {selectedFood.cal} kcal · {selectedFood.prot}g P · {selectedFood.carb}g C · {selectedFood.fat}g F</div>
                </div>
                <input className="inp" type="number" placeholder="Portion size (grams)" value={portionG} onChange={e=>setPortionG(e.target.value)} autoFocus min="1"/>
                {portionG&&parseFloat(portionG)>0&&(
                  <div style={{background:'rgba(22,163,74,.07)',border:'1px solid rgba(22,163,74,.18)',borderRadius:7,padding:'8px 11px',marginBottom:8}}>
                    <span className="mono" style={{fontSize:11,color:'#16A34A'}}>→ {Math.round(selectedFood.cal*portionG/100)} kcal · {Math.round(selectedFood.prot*portionG/100)}g P · {Math.round(selectedFood.carb*portionG/100)}g C · {Math.round(selectedFood.fat*portionG/100)}g F</span>
                  </div>
                )}
                <select className="sel" value={searchMeal} onChange={e=>setSearchMeal(e.target.value)}>
                  {MEALS.map(m=><option key={m} value={m}>{MEAL_ICONS[m]} {m.charAt(0).toUpperCase()+m.slice(1)}</option>)}
                </select>
                <div className="row-btns"><button className="btn btn-ghost btn-sm" onClick={()=>{setSelectedFood(null);setPortionG('');}}>← Back</button><button className="btn btn-gold btn-sm" onClick={addSearchedFood} disabled={!portionG||parseFloat(portionG)<=0}>Add Food</button></div>
              </>
            )}
            {addMode==='manual'&&(
              <>
                <input className="inp" placeholder="Food name…" value={manualFood.name} onChange={e=>setManualFood(f=>({...f,name:e.target.value}))} autoFocus/>
                <input className="inp" type="number" placeholder="Calories (kcal) *" value={manualFood.calories} onChange={e=>setManualFood(f=>({...f,calories:e.target.value}))} min="0"/>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:7,marginBottom:8}}>
                  {[['protein','Protein (g)'],['carbs','Carbs (g)'],['fat','Fat (g)']].map(([k,ph])=>(
                    <input key={k} className="inp" type="number" placeholder={ph} value={manualFood[k]} onChange={e=>setManualFood(f=>({...f,[k]:e.target.value}))} style={{margin:0}} min="0"/>
                  ))}
                </div>
                <select className="sel" value={manualFood.meal} onChange={e=>setManualFood(f=>({...f,meal:e.target.value}))}>
                  {MEALS.map(m=><option key={m} value={m}>{MEAL_ICONS[m]} {m.charAt(0).toUpperCase()+m.slice(1)}</option>)}
                </select>
                <div className="row-btns"><button className="btn btn-ghost btn-sm" onClick={()=>setShowAddFood(false)}>Cancel</button><button className="btn btn-gold btn-sm" onClick={addManualFood} disabled={!manualFood.name.trim()||!manualFood.calories}>Add Food</button></div>
              </>
            )}
          </div>
        </div>
